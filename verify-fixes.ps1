...
if (Test-Path "src/app/app-routing.module.ts") {
    ...
    if ($routingContent -match "loadComponent.*logs\.component") {
...
if ($allCorrect) {
    ...
    Write-Host '   (Get-Content src/app/app-routing.module.ts) -replace "loadChildren.*logs/logs.module", "loadComponent: () => import('"'"'./modules/logs/logs.component'"'"')" | Set-Content src/app/app-routing.module.ts' -ForegroundColor Gray
    Write-Host ""
}
